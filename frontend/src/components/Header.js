...
import { LogIn, LayoutDashboard, ShieldCheck, LogOut, UserPlus, Heart, GitCompareArrows } from "lucide-react";
    ...
    <header className="bg-white border-b border-gray-200">
      <div className="container-x flex items-center justify-between h-16">
        ...
        <nav className="flex items-center gap-1 md:gap-3">
          <NavLink to="/fahrzeuge" data-testid="nav-marketplace" className={({isActive}) => `swiss-label px-3 py-2 ${isActive ? "text-[#16A34A]" : "text-gray-700 hover:text-[#0A0A0A]"}`}>
            Fahrzeuge
          </NavLink>
          ...
          {user && (
            <button data-testid="logout-btn" onClick={onLogout} className="inline-flex items-center gap-2 text-sm font-semibold text-gray-700 hover:text-[#16A34A] px-3 py-2">
...
function NavBadgeLink({ to, testid, icon: Icon, label, count, filled = false, badgeTestid }) {
  return (
    <NavLink to={to} data-testid={testid} className={({isActive}) => `relative inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold ${isActive ? "text-[#16A34A]" : "text-gray-700 hover:text-[#0A0A0A]"}`} title={label}>
      <Icon className="w-4 h-4" fill={filled && count > 0 ? "currentColor" : "none"} />
      <span className="hidden md:inline">{label}</span>
      {count > 0 && (
        <span data-testid={badgeTestid} className="ml-0.5 min-w-[18px] h-[18px] px-1 text-[10px] font-bold bg-[#16A34A] text-white inline-flex items-center justify-center">{count}</span>
      )}
    </NavLink>
  );
}
